...
async fn ls_handler(Json(input): Json<LsRequest>) -> Result<Json<Vec<FileEntry>>, String> {
...
async fn read_handler(Json(input): Json<ReadRequest>) -> Result<Json<serde_json::Value>, String> {
    let path = normalize_path(&input.path);
    if !path.exists() {
...
async fn write_handler(Json(input): Json<WriteRequest>) -> Result<Json<serde_json::Value>, String> {
    let path = normalize_path(&input.path);
...
async fn mkdir_handler(Json(input): Json<MkdirRequest>) -> Result<Json<serde_json::Value>, String> {
    let path = normalize_path(&input.path);
...
async fn rm_handler(Json(input): Json<RmRequest>) -> Result<Json<serde_json::Value>, String> {
    let path = normalize_path(&input.path);
    if !path.exists() {
...
async fn mv_handler(Json(input): Json<MvRequest>) -> Result<Json<serde_json::Value>, String> {
...
async fn cp_handler(Json(input): Json<CpRequest>) -> Result<Json<serde_json::Value>, String> {
    let source = normalize_path(&input.source);
    let dest = normalize_path(&input.destination);
...
async fn stat_handler(Json(input): Json<StatRequest>) -> Result<Json<StatResult>, String> {
    let path = normalize_path(&input.path);
    if !path.exists() {
        return Err("Path not found".to_string());
    }
...
async fn search_handler(Json(input): Json<SearchRequest>) -> Result<Json<Vec<FileEntry>>, String> {
...
async fn download_handler(Json(input): Json<ReadRequest>) -> Result<impl IntoResponse, String> {
